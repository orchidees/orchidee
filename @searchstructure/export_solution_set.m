...
% EXPORT_SOLUTION_SET - Export current orchestration solutions in a
% structured text file.
%
% Usage: export_solution_set(searchstructure_instance,knowledge_instance,filename,<handles>)
%
...
for i = 1:size(searchstructure_instance.solution_set.genes,1)
    ...
    for j = 1:size(searchstructure_instance.solution_set.genes,2)
        if searchstructure_instance.solution_set.genes(i,j) == searchstructure_instance.feature_structure.neutral_element
            ...
            dbname = get_field_values(knowledge_instance,'dbname',db_idx); dbname = dbname{1};
            file = get_field_values(knowledge_instance,'file',db_idx); file = file{1};
            fprintf(fid,'/%s%s%s ',dbname,dir,file);
    ...
    for j = 1:length(searchstructure_instance.used_features)
        feature_name = searchstructure_instance.used_features{j};
        feature_values = searchstructure_instance.solution_set.features.(feature_name);
        feature_str = mat2str(feature_values(i,:));
